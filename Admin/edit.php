...
if ($_POST[submit]) {
$sql = "UPDATE $pagestable SET title = '$_POST[titlepage]', page = '$_POST[news]' WHERE page_id = '$_POST[edit]'";
$result = mysql_query($sql);
echo "<p class='header'>Page edited.</p>";
}
else {
$sql="SELECT * FROM $pagestable WHERE page_id = '$_GET[edit]' ORDER BY page_id DESC";
$result=mysql_query($sql,$db);
...
<tr>
<td><input type="Text" size="45" name="titlepage" style="background-color: <?echo"$color5" ?>; border: 1 solid <?echo"$color1" ?>" class="text" value="<?echo "$row[title]" ?>"></td>
</tr>
<tr>
<td><p class="text">Text:</p></td>
</tr>
<tr>
<td><textarea name="news" cols="45" rows="10" wrap="VIRTUAL" style="background-color: <?echo"$color5" ?>; border: 1 solid <?echo"$color1" ?>" class="text"><?echo "$row[content]" ?></textarea></td>
</td>
</tr>
<tr>
<td>

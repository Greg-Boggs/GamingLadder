...
if (isset($_POST['rerank']) || ((isset($_GET['batch'])) && (isset($_GET['start'])))) {
    ...
    if (!isset($_GET['entries'])) {
        $sqlcount = mysqli_query($db, "SELECT count(*) FROM $gamestable");
        $numbergames = mysqli_fetch_row($sqlcount);
        $NumberGamesInTotal = $numbergames[0];
    }

    // If game entries exist and we have them in the URL we can read them from there.
    if ($_GET['entries'] > 0) {
        $NumberGamesInTotal = $_GET['entries'];
    }
    ...
    $rpresult = mysqli_query($db, $rpquery) or die (mysqli_error($db));
...
// While there is content in the batch we selected we will re-rank every game in it _one by one_. First a game is _rated_, then if enabled, it is also _ranked_ according to whatever ruleset happens to exist in the current config file. This while.loop contains most of the work done in this file.
    ...
    while ($rprow = mysqli_fetch_array($rpresult, MYSQLI_ASSOC)) {
        ...
        if (RERANK_CREATES_RANK_HISTORY == 1) {
            ...
            if ($rowwrank['rank'] != 0) {
                ...
                $playedpast = mysqli_query($db, "SELECT COUNT( * ) FROM $gamestable WHERE (winner = '$winner' OR loser = '$winner') AND reported_on < '" . $rprow['reported_on'] . "' AND reported_on > '" . $rprow['reported_on'] . "' - INTERVAL $passivedays DAY") or die(mysqli_error($db));
                $xplayed = mysqli_fetch_row($playedpast);
            ...
            if ($rowlrank['rank'] != 0) {
                ...
                $playedpast = mysqli_query($db, "SELECT count(*) FROM $gamestable WHERE (winner = '$loser' OR loser = '$loser') AND reported_on < '" . $rprow['reported_on'] . "' AND reported_on > '" . $rprow['reported_on'] . "' - INTERVAL $passivedays DAY") or die(mysqli_error($db));
                $xplayed = mysqli_fetch_row($playedpast);
                $xplayedclean = $xplayed[0];
            ...
            mysqli_query($db, "INSERT INTO $standingscachetable 
			...
			) a JOIN $gamestable g ON (g.reported_on = a.latest_game)) standings") or die(mysqli_error($db));
            ...
            if ($AdminIsReRankingTheWholeThing == 1) {
                mysqli_query($db, "DROP TEMPORARY TABLE IF EXISTS $databasename.temp_played");
            }
...
} else if (isset($_POST['recache']) && $_POST['recache']) {
...
// Recache the ladder, it takes about 1-2 seconds with 25000 games
    ...
    <form name="form1" method="post" action="rerankladder.php">
        ...
        <p>
        <ul>
            <li>If the cache is corrupted and the ladder rankings/info seem wrong select <i>Update Corrupted Cache. The
                    process is pretty fast and painless.</i></li>
            <li>If you have changed some settings in the config file that involve anything that effects how a player's
                rated, activity, xp etc is counted then those settings only have effect for the games reported
                <i>after</i> you made changed them. In order to make already played games use the same ruleset as your
                new settings in the config file (after you changed it) you must re-rank the whole ladder. To do that,
                please select <i>Rerank Ladder</i>, and be sure to have set up the re-rank process properly in the
                config file before you do this. This process may take some time and can time out if you have set it up
                wrong, but the core data can't be damaged by failures and it can be repeated until you succeed.
        </ul>
        </p>
        <input type="submit" name="rerank" value="Rerank Ladder" class="text"
               onclick="return confirm('Are you sure you want to rerank the ladder?');">
        <input type="submit" name="recache" value="Update corrupted cache" class="text"
               onclick="return confirm('Are you sure you want to redo the cache?');">
    </form>
    <?php
}
require('../bottom.php');

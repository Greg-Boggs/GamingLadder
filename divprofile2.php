...
//if (($loggedin == 1) && ($_GET[name] == $nameincookie)) {
	...
	// check how many passive days we allow.. it depens on what division the player is in so we frist need to take the rating to get to know the division...

// echo "<h1>$bajs['rating']</h1>";

	if ($bajs[rating] <= $ladderminelod2) {
	// this happens when the rating is lower than the one needed to enter division 1

	//echo "<h1>$bajs[rating] / $ladderminelod2</h1>";
	$daysleft = $passivedaysd2 - $daysago;
	//echo "<h1>1. $daysleft</h1>";

// Display the number of days left before setting the player in passive mode...
	$passivedate = ($unixedlatest + ($passivedaysd2*24*60*60));

	} else {

	// okey, here the players that have better rating get their evalution fo inactivity. they have a harsher setting ; ) 
...
while ($jaja[name] != $_GET[name]) {
	...
	if ($bajs[rating] <= $ladderminelod2) {
		...
		//echo "<h1>$bajs[rating] / $ladderminelod2</h1>";
		$daysleftothers = $passivedaysd2 - $daysago;
		...
		// okey, here the players that have better rating get their evalution fo inactivity. they have a harsher setting ; ) 
		$daysleftothers = $passivedays - $daysago;
		$passivedate = ($unixedlatest + ($passivedays*24*60*60));
	}
	...
	if ($bajs[rating] <= $ladderminelod2) {
		...
		if (($jaja[name] != "zAdminTest03") && ($jaja[name] != "zAdminTest") &&  ($jaja[totalgames] >= $gamestorankd2) && ($daysleftothers >= 0) && ($jaja[rating] <= $ladderminelod2)) {
				
				$rank = $rank+1;
				// deb echo "$jaja[name] $rank $jaja[LastGame] $daysleftothers <br>";
		}
	} else {
	
	
		if (($jaja[name] != "zAdminTest03") && ($jaja[name] != "zAdminTest") &&  ($jaja[totalgames] >= $gamestorank) && ($daysleftothers >= 0)) {
			
			$rank = $rank+1;


			//echo "$jaja[name] $rank $daysleft <br>";
		}
	}
...
$jabberpic = "<img border='1' src='icons/jabber.gif' align='absmiddle'>";
...
$icqnumber = "$row[icq]";
$icqpic = "<img border='1' src='icons/icq.gif' align='absmiddle'>";
}
if ($row[aim] == "n/a") {
$aimname = "n/a";
$aimpic = "";
...
$aimname = "$row[aim]";
$aimpic = "<img border='1' src='icons/aim.gif' align='absmiddle'>";
}
if ($row[msn] == "n/a") {
$msnname = "n/a";
$msnpic = "";
...
if (($row[rating] <= $ladderminelod2) && ($row[totalgames] < $gamestorankd2)) {
echo "unranked"; }

if (($row[rating] >= $ladderminelo) && ($row[totalgames] < $gamestorank)) {
echo "unranked"; }
...
if ($row[CanPlay] != "") { ?>
	...
	<tr>
		...
		if ($pos1 != FALSE) {echo "<img border=\"0\" src=\"icons/streakplus.gif\">";}?></td>

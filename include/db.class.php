...
class DB
{
    ...
    */
    ...
    {
        $this->db_handle = mysqli_connect($config->get_databaseserver(), $config->get_databaseuser(), $config->get_databasepass());
        mysqli_select_db($this->db_handle, $config->get_databasename());
    ...
    */
    public function execute($query = NULL)
    {
        ...
        //if $query is set, then we execute query localy, using mysqli_query function....
        //If $query is not set, then we call execute method of current query object...
        $result = (isset($query)) ? mysqli_query($this->db_handle, $query) : $this->query->execute($this->db_handle);
        $mysqli_error = mysqli_error($this->db_handle);
        if ($mysqli_error) {
    ...
    */
    ...
    {
        return mysqli_fetch_assoc(((isset($query)) ? $this->execute($query) : $this->query->execute($this->db_handle)));
    }
    ...
    */
    ...
    {
        return mysqli_num_rows(((isset($query)) ? $this->execute($query) : $this->query->execute($this->db_handle)));
    }
...
class DB_Query
{
    ...
    */
    public function execute($mysql_handle)
    {
        ...
        $result = mysqli_query($mysql_handle, $this->to_string());
        $mysqli_error = mysqli_error($mysql_handle);
        if ($mysqli_error) {
            throw new Exception($mysqli_error);
        } else {
            return $result;

...
//reading the form input
	...
	//check for an existing teamname
	      ...
	      if ($samenick == 1) { echo "Sorry but it seems someone was faster in picking this name. Try another one.";}
	...
	//teampartner checking
		...
		$i = 0;
		while ($i < NUMBER_MEMBERS){
		if ($names[$i] == "") {echo "Sry, but you have to chose for every spot a teammates.";echo "<br />";} // just counts the elements of $names vs the required number NUMBER_MEMBERS
		$i++;}
	    
		//check duplicates
		if (array_unique($names) != $names){echo "Sry, but a team should consist of (at least) ".NUMBER_MEMBERS." <b>different</b> people";echo "<br />";}
		else{
		//check if all of them are registered
		  ...
		  if (count($notregistered) > 0){
		    ...
		    {echo "$value <br />";}
		    echo "couldnt be found in the database, be sure only to chose registered players.";echo "<br />";} 
	...
	// if all checkings are passed write the team to the database
		  ...
		  $sql = $query;
		  $result = mysql_query($sql,$db);
		  if ($result) {echo "Success! Your team is now registered in the database.";}
		    else {echo "something was wrong with the sql query, couldnt write team into database";}
...
<tr>
  ...
  <td>&nbsp;<input type="Text" name="teamname" class="text"> (<?php echo REG_MIN_NICKLENGTH . " - " . REG_MAX_NICKLENGTH . " ";?> char)</td>
</tr>
<!-- Team-Avatar Selection -->
<tr><td><p class="text"><b>Team-Avatar</b></p></td>
